...
bool
sblPlanner::
Query(const mpkConfig& qs, const mpkConfig& qg, list<mpkConfig> &genPath,
...
#ifdef WIN32
  struct _timeb start, end; // to take time for planning
#else
...
#ifdef ADAPT_COLLCHECKER
    new_node.adapt_point_checker = new mpkConfigChecker(test_pairs_, robots_);
#endif
...
bool
sblPlanner::
setRoot(const mpkConfig& q, sblTree& T)
{    
  sblMilestone m(nodes_counter, q);
  total_CC++; // Update the counter of coll checks

#ifdef ADAPT_COLLCHECKER
  m.adapt_point_checker = new mpkConfigChecker(test_pairs_,robots_);
...
sample(const sblMilestone& m, sblMilestone& new_node, double rho)
{
  ...
  do{
    randConfig.LocalBoxSample(m.getConfig(),rho_local,&conf);
...
void
sblPlanner::
report_times(struct _timeb start, struct _timeb end)
{
  if (end.millitm >= start.millitm) {
    int secs = end.time - start.time;
    int msecs = end.millitm - start.millitm;
    double diff = (double)secs + ((double)msecs)/1000.0;
    cerr << "\n" << diff << "\t";
  }
  else {
    int secs = end.time - start.time - 1;
    int msecs = 1000 - start.millitm + end.millitm;
    double diff = (double)secs + ((double)msecs)/1000.0;
    cerr << "\n" << diff << "\t";
  }
}
...
void
sblPlanner::
addTime(struct _timeb start, struct _timeb end)
{
  int secs, msecs;
  double diff;
  ...
  if (end.millitm >= start.millitm) {
    ...
    msecs = end.millitm     - start.millitm;
    diff    = (double)secs + ((double)msecs)/1000.0;
  }
  else {
    secs = end.time - start.time - 1;
    msecs = 1000 - start.millitm + end.millitm;
    diff    = (double)secs + ((double)msecs)/1000.0;
  }
  CC_time += diff;
}
...
#endif
{
  ...
  cerr<<Ti.getSize()+Tg.getSize()<<"\t"<<freePath.size()<<"\t"\
      <<total_CC<<"\t"<<path_CC<<"\t"<<sampled_Mil<<"\t"<<CC_time;
